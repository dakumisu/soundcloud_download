...
const run = async () => {
  for (const url of argv.url) {
    ...
    await sleep(500); // slight delay between playlists
  }
...
async function handlePlaylist(playlistUrl) {
  ...
  return new Promise((resolveMeta) => {
    meta.on('data', (data) => {
      ...
      albumData.title = rawOutput.match(/"album": "(.*?)"/)?.[1] || '';
      albumData.artist = rawOutput.match(/"album_artist": "(.*?)"/)?.[1] || '';

      const defaultName = playlistUrl.split('/').filter(Boolean).pop().split('?')[0];
      const folderName = argv.folder || albumData.title || defaultName || 'Playlist';
      folder = sanitize(folderName);
			console.log('------------ FOLDER ------------');
			console.log(folder);
			console.log('--------------------------------');
